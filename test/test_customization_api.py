...
def test_a_bare_callable_is_the_simple_form(clean_registry):
    ...
    warnings, actions_n, hooks_n, _, _ = user_api.load_user_entries(config_with(ACTIONS={"mine": mine}))
    assert warnings == []
...
def test_shadowing_a_builtin_needs_an_explicit_override(clean_registry):
    ...
    warnings, actions_n, _, _, _ = user_api.load_user_entries(config_with(ACTIONS={"quit": mine}))
    assert actions_n == 0
    assert len(warnings) == 1 and "override" in warnings[0]
    assert xa.registry.resolve(xa.FILER, "quit").source == "builtin"

    warnings, actions_n, _, _, _ = user_api.load_user_entries(
...
def test_hooks_load_and_unknown_events_warn(clean_registry):
    ...
    warnings, _, hooks_n, _, _ = user_api.load_user_entries(config_with(
